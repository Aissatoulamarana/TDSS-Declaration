...
export function JobNewEditForm({ currentJob }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      reset();
      toast.success(currentJob ? 'Mise à jour reussie!' : 'Création avev succes!');
  ...
  /* const renderProperties = (
    <Card>
      ...
      <Stack spacing={3} sx={{ p: 3 }}>
        <Stack spacing={1}>
          <Typography variant="subtitle2">Type Emploi</Typography>
          <Field.MultiCheckbox
            row
  ...
  const renderActions = (
    <Box display="flex" alignItems="center" flexWrap="wrap">
      ...
      <LoadingButton
        ...
        sx={{ ml: 2 }}
      >
        {!currentJob ? 'Creer un Job' : 'Sauvegarder les changements'}
      </LoadingButton>
    </Box>
  );
