      ...
      .map((group, index) => (
        <Box component="ul" key={`${group}-${index}`}>
          {dataGroups[group].map((item) => {
            ...
            // Check if title and path are defined
            if (!title || !path) {
              console.error('Invalid item:', item); // Log the invalid item
              return null; // Skip rendering this item
            }

            const titleMatches = match(title, searchQuery);
            const pathMatches = match(path, searchQuery);

            const partsTitle = parse(title, Array.isArray(titleMatches) ? titleMatches : []);
            const partsPath = parse(path, Array.isArray(pathMatches) ? pathMatches : []);
